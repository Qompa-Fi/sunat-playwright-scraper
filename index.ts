  ...
  const app = new Elysia()
    ...
    .get(
      ...
      async ({ query: credentials, error }) => {
        const token = await getSunatToken(browser, credentials);
        if (!token) {
          log.error("sunat token could not be retrieved");
          return error(500, "Internal Server Error");
        }

        const data = await getExportedData(token, "080000");
        return data;
      },
    ...
    .get(
      ...
      async ({ query: credentials, error }) => {
        ...
        const periods = await getPeriods(token, "080000");
        return {
          tax_periods: periods,
        };
      },
    ...
    .get(
      ...
      async ({ query: credentials, error }) => {
        ...
        const data = await getExportedData(token, "140000");
        return data;
      },
      { query: credentialsSchema },
    )
    .get(
      ...
      async ({ query: credentials, error }) => {
        ...
        const periods = await getPeriods(token, "140000");
        return {
          tax_periods: periods,
        };
      },
